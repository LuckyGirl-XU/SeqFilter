...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, mode:str='val'):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if (batch_idx+1) % 100 == 0:
                print(f'evaluate for the {batch_idx + 1}-th batch, evaluate loss: {loss.item()}')
...
def evaluate_model_node_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        evaluate_metrics = get_node_classification_metrics(
...
def evaluate_edge_bank_link_prediction(args: argparse.Namespace, train_data: Data, val_data: Data, test_idx_data_loader: DataLoader,
    ...
    for run in range(args.num_runs):
        ...
        for batch_idx, test_data_indices in enumerate(test_idx_data_loader_tqdm):
            ...
            predicts = torch.from_numpy(np.concatenate(
                [positive_probabilities, negative_probabilities])).float()
            labels = torch.cat([torch.ones(len(positive_probabilities)), torch.zeros(
                len(negative_probabilities))], dim=0)

            loss = loss_func(input=predicts, target=labels)

            test_losses.append(loss.item())

            test_metrics.append(get_link_prediction_metrics(
                predicts=predicts, labels=labels))

            # test_idx_data_loader_tqdm.set_description(f'test for the {batch_idx + 1}-th batch, test loss: {loss.item()}')
            if (batch_idx + 1) % 100 == 0:
                print(f'test for the {batch_idx + 1}-th batch, test loss: {loss.item()}') 
        ...
        for metric_name in test_metrics[0].keys():
            ...
            logger.info(f'test {metric_name}, {average_test_metric:.8f}')
        ...
        # save model result
        result_json = {
            "test metrics": {metric_name: f'{test_metric_dict[metric_name]:.8f}'for metric_name in test_metric_dict}
        }
